...
class BaseSelector:
    def download_dataset(self, obj, n_threads, geojsonFolder, annotationFolder, imagesFolder, custom_signals, check):
        if check == 0:
            ...
            geojson_files = [f for f in os.listdir(geojsonFolder) if
                             f.endswith('.geojson')]  # Assicurati che siano file geojson
...
class NumberSelector(BaseSelector):
        def _select_map_features(self, obj, geojsonFolder):
            ...
            try:  # Gestisci file non trovati o non validi
                ...
                if mList:
                    try:  # Gestisci ValueError se 'number' è maggiore di len(mList)
                        mapF_id_list = random.sample(mList, obj.selector['number'])
                    except ValueError:
                        print(
                            f"Il file {geojsonFolder} contiene meno feature del numero richiesto. Seleziono tutte le feature disponibili.")
                        mapF_id_list = mList  # Seleziona tutte le feature disponibili

            except (
            FileNotFoundError, json.JSONDecodeError):  # Gestisci l'eccezione se il file non esiste o non è valido
                print(f"Errore durante l'apertura o la decodifica di {geojsonFolder}.")
...
class Dataminer:
    ...
    def getCustomConfiguration(self, configurationFolder):
        """Ottiene la configurazione personalizzata."""
        with open(configurationFolder) as file:
            self.lines = [line.rstrip('\n') for line in file]

    def getNGeojson(self, filepath):
        """Ottiene il numero di elementi GeoJSON, handling potential errors."""
        try:
            with open(filepath, 'r') as f:
                try:  # Inner try-except for JSON loading
                    temp = geojson.load(f)
                    count = len(temp.get('features', []))
                    return count
                except json.JSONDecodeError:
                    print(f"Error decoding GeoJSON file: {filepath}")
                    return 0
        ...
        for row in range(rows):
            for col in range(cols):
                ...
                try:
                        ...
                        for y in range(min(cell_lly, cell_ury), max(cell_lly, cell_ury) + 1):
                            ...
                            if self.Type == "custom":
                                if configurationFolder == "empty":
                                    raise ValueError("with custom configuration you need to add a configuration folder")
                    ...
                    print(f"Errore durante il download delle tile: {e}")
    ...
    def process_data(self, map_feature_id, lock, outputFolderImages, outputFolderAnnotations, custom_signals):
        ...
        if image_id is None:
            return
        print(f"Immagine selezionata: {image_id}, Distanza: {image_distance:.2f}m")

        url = 'https://graph.mapillary.com/{}?fields=thumb_original_url, geometry'.format(image_id)
        response = requests.get(url, headers=header)
        data = response.json()
        ...
        if download_image:
            with open('{}/{}.jpg'.format(outputFolderImages, map_feature_id), 'wb') as handler:
                image_data = requests.get(image_url, stream=True).content
                handler.write(image_data)

            with open('{}/{}.json'.format(outputFolderAnnotations, map_feature_id), 'w') as handler:
    ...
    def downloadDataSet(self, n_threads, geojsonFolder, annotationFolder, imagesFolder,
                        download_strategy: MapFeatureSelector, custom_signals=None, check=-1):
    ...
    def getDistance(self, coordinatesMapF, imageIdList):
        ...
        for image_id in imageIdList:
            ...
            response = requests.get(url, headers=header)
            data = response.json()
            coordinatesImageId = data['geometry']['coordinates']
